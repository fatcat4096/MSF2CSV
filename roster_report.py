...
async def get_roster_report(
	...
	alliance_name = await lookup_alliance(self, context)
	...
	if report_in_progress(alliance_name, output_name, start_time) != start_time:
		return await send_rosterbot_error(self, context, f"**/roster {output_name}** for **{alliance_name}** already in progress.\nCommand cancelled.")
	...
	if STALE_DATA and not DISABLE_REFRESH:
		...
		#
		# NEED TO WRITE CODE TO GET AUTH BASED ON ALLIANCE_NAME AND CHANNEL_ID / USER_ID
		# AS PART OF THIS PROCESS, IT WOULD RETRIEVE THE CURRENT ALLIANCE NAME
		# IF ALLIANCE NAME HAS CHANGED AND REQUEST NOT FROM OWNER, RETURN NULL
		#
		# AT END OF ROSTER REFRESH, IF NAME HAS CHANGED AND PLAYERS ARE SIMIILAR,
		# MERGE OLD ALLIANCE INFO INTO NEW **AND** UPDATE THE ALLIANCE NAME ON ALL ENTRIES IN 
		# DEFAULT TABLE THAT THIS PLAYER HAS GIVEN OUT (i.e. OLD alliance_name AND THIS USER'S discord_id)
		#
		
		AUTH = await get_available_auth(self, context, alliance_name)
		...
		if AUTH:
			...
			message, embed = await get_roster_refresh(self, context, alliance_name, report, message, AUTH)
	"""
