...
def find_selection(item_list, selection):
	...
	# Exact match!
	if selection in item_list or not selection:
		return selection

	# Sloppy search for list
	if type(item_list) is list:
		for idx,option in enumerate(item_list):
			# Found a match!
			if ''.join(selection.lower().split()) in ''.join(option.lower().split()):
				return item_list[idx]

	# Sloppy search for dicts
	elif type(item_list) is dict:
...
async def send_rosterbot_embed(
	...
	content =  '_ _' if cont else "Here's the info you requested..." 
...
def get_driver_and_login(alliance_name=None, fresh=False, roster_refresh=False, scopely_login='', alliance_info={}, log_file=None, headless=True):
	...
	#
	# WE PREVIOUSLY DIDN'T DO A DRIVER POOL CHECK IF A SCOPELY_LOGIN WAS PROVIDED
	# IF WE NOW HAVE STRUCTURES THAT TIE LOGIN TO ALLIANCE NAME (auth_by_login)
	# WE SHOULD LOOK LOGIN UP AND USE CURRENT ALLIANCE ASSOCIATION
	#
...
def get_set_defaults(context, other_options, locals):
