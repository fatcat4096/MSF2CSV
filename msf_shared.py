...
CLIENT_TOKEN = msf2csv.construct_token(getenv('CLIENT_ID'), getenv('CLIENT_SECRET'))
...
async def get_auth(self, context):	
	...
	if AUTH and not msf2csv.auth_valid(AUTH):
		...
		if AUTH.get('updated'):
			await self.bot.database.save_auth(context.author.id, AUTH)	
...
async def api_request_failed(self, context, response, description):
	...
	embed = await get_rosterbot_embed(self, context, title='API Request Failed:', description=description + '\n**API request failed.** Command cancelled.', color=build_color(0.01))
...
async def send_rosterbot_embed(
		self, context: Context, title: str = '', description: str = '', cont : bool = False, color = None
	) -> None:
	...
	await context.send(embed=embed, content=content)
...
def load_cached_members(alliance_name):
	...
	alliance_members['members'] = {member:{'discord':member_info[member].get('discord',{}), 'scopely':member_info[member].get('scopely',''), 'url':member_info[member].get('url')} for member in sorted(member_info, key=str.lower)}
...
async def get_recruit_info(self, context, alliance_name, alliance_url, recruit_info):
	...
	if selected_member:
		...
		await loop.run_in_executor(None, partial(msf2csv.process_rosters, driver, recruit_info, only_process=[recruit_info['name']], log_file=log_file))
...
def get_driver_and_login(alliance_name=None, fresh=False, roster_refresh=False, scopely_login='', alliance_info={}, log_file=None, headless=True):
	...
	# WE NO LONGER EVER KEEP ANY DRIVER OPEN AFTER COMPLETION. 
	# * SO THERE IS NO AVAIL POOL. 
	# * ACTIVE_POOL INCLUDES JOBS CURRENTLY IN-FLIGHT (STILL LIMIT TO ONE REQUEST PER ALLIANCE AT A TIME)
	# * POOL_QUEUE INCLUDES JOBS REQUESTING A DRIVER FOR AN ALLIANCE ALREADY IN USE.
	# IF A USER REQUESTS RECRUIT, IT WILL USE MY DEFAULT LOGIN IN A TEMP SESSION, NOT PRE-AUTH SESSION

	# Do one workflow for scopely_login is defined, which uses get_session_by_login()
	# Do another workflow for alliance_name is define which uses get_session_by_alliance()


	# If we provided a scopely_login, need to take a slightly different workflow.
	if scopely_login:

		# BUILD THE NEW MECHANISM. START WITH THOSE SUPPLIYING SCOPELY_LOGIN. 
		# If session is defined, will use new mechanism.

		session = None

		# USE NEW MECHANISM FOR ONLY ME -- NEED TO FIND A VALID SESSION TO MAKE THE REQUEST IN 
		if scopely_login in ('lousassle4321@gmail.com'):
			pass
			# GET A VALID SESSION HERE USING OUR AUTH_BY_LOGIN FRAMEWORK
			# THIS IS READ ONLY -- JUST GET A RECENT AUTH SESSSION FROM AUTH_BY_LOGIN IF ONE EXISTS AND THE SESSION POOL SHOWS THE SESSION AS FREE
			# AUTH_BY_LOGIN WILL BE UPDATED **AFTER** LOGIN 
